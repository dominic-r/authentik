...
class TestSourceStage(FlowTestCase):
    ...
    def test_source_save(self):
        """Test source saving functionality"""
        user = create_test_user()
        flow = create_test_flow(FlowDesignation.AUTHENTICATION)
        stage = SourceStage.objects.create(name=generate_id(), source=self.source)
        FlowStageBinding.objects.create(
        ...
        # Verify the source is saved correctly
        self.source.refresh_from_db()
        self.assertEqual(self.source.slug, generate_id())
        self.assertEqual(self.source.issuer, "authentik")
        self.assertTrue(self.source.allow_idp_initiated)
        self.assertEqual(self.source.pre_authentication_flow, create_test_flow())
